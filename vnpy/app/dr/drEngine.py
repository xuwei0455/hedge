...
from drBase import *
from vnpy.engine.vt.vtObject import VtTickData, VtBarData
...
class DrEngine(object):
    ...
    def procecssTickEvent(self, event):
        ...
        if vtSymbol in self.tickDict:
            ...
            # 发出日志
            self.writeDrLog(u'记录Tick数据{symbol}，时间:{time}, last:{last}, bid:{bid}, ask:{ask}'
                            .format(symbol=tick.vtSymbol,
                                    time=tick.time,
                                    last=tick.lastPrice,
                                    bid=tick.bidPrice1,
                                    ask=tick.askPrice1))
        ...
        if vtSymbol in self.barDict:
            ...
            if not bar.datetime or bar.datetime.minute != tick.datetime.minute:
                if bar.vtSymbol:
                    ...
                    self.writeDrLog(u'记录分钟线数据{symbol}，时间:{time}, O:{open}, H:{high}, L:{low}, C:{close}'
                                    .format(symbol=bar.vtSymbol,
                                            time=bar.time,
                                            open=bar.open,
                                            high=bar.high,
                                            low=bar.low,
